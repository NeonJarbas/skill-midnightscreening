...
class MidnightScreeningSkill(OVOSCommonPlaybackSkill):
    ...
    def initialize(self):
        url = "https://www.youtube.com/c/TheMidnightScreening"
    ...
    def match_skill(self, phrase, media_type):
        ...
        if self.voc_match(phrase, "movie") or media_type == MediaType.MOVIE:
            score += 10
        if self.voc_match(phrase, "midnight"):
            score += 50
        return score

    def normalize_title(self, title):
        title = title.lower().strip()
        title = self.remove_voc(title, "midnight")
    ...
    def get_playlist(self, score=50, num_entries=250):
        ...
        return {
            ...
            "title": "The Midnight Screening (Movie Playlist)",
            "author": "The Midnight Screening"
        }

    @ocp_search()
    def search_db(self, phrase, media_type):
        base_score = self.match_skill(phrase, media_type)
        if self.voc_match(phrase, "midnightscreening"):
